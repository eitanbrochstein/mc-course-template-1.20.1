...
public class MetalDetectorItem extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        if (!context.getWorld().isClient()) {
            ...
            if (!foundBlock) {
                player.sendMessage(Text.translatable("item.mc-course.metal_detector.no_valuables"));
            }
        }
    ...
    private void addNbtDataToDataTablet(PlayerEntity player, BlockPos position, Block block) {
        ...
        nbtData.putString("mc-course.last_valuable_found", "Valuable Found: " + block.getName().getString() + " at " +

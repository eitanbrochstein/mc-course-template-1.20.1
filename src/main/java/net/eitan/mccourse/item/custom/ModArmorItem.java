...
import net.eitan.mccourse.item.ModArmorMaterial;
...
public class ModArmorItem extends ArmorItem {
    ...
    private void evaluateArmorEffects(PlayerEntity player) {
        for (Map.Entry<ArmorMaterial, StatusEffectInstance> entry : MATERIAL_TO_EFFECT.entrySet()) {
    ...
    public boolean hasFullSuitofArmor(PlayerEntity player) {
        ItemStack boots = player.getInventory().getArmorStack(0);
        ItemStack leggings = player.getInventory().getArmorStack(1);
        ItemStack chestplate = player.getInventory().getArmorStack(2);
        ItemStack helmet = player.getInventory().getArmorStack(3);

        return !boots.isEmpty() && !leggings.isEmpty() && !helmet.isEmpty() && !chestplate.isEmpty();
    }
}

package net.eitan.mccourse.world.tree.custom;
...
public class DriftwoodFoliagePlacer extends FoliagePlacer {
    ...
    protected void generate(TestableWorld world, BlockPlacer placer, Random random, TreeFeatureConfig config, int trunkHeight, TreeNode treeNode, int foliageHeight, int radius, int offset) {
        // create the foliage
        // treeNode.getCenter() is the first position of the last log placed!
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // y how much offset in the y direction from attachment.pos()
            // y if it is dependent on i, also offsets each new layer in the y direction

            generateSquare(world, placer, random, config, treeNode.getCenter().up(i), 1, i + 1, treeNode.isGiantTrunk());
        }
    }
